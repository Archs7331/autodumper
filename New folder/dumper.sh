...
while /bin/true; do
  ...
  pkt=$(( $pkt_new - $pkt_old ))
  echo -ne "\r[40;34m[[40;35m+[40;34m] ArchsDumper V1 - Paid/Private Auto-Dumper | WebHook Noti | Auto-Reporter | Interface: $interface | Live PPS: $pkt |[40;34m[[40;35m+[40;34m][40;37m"
   if [ $pkt -gt 450 ]; then
    echo -e "\n`date` Attack detected Gathering Info"
  dateinfo=`date +"%d-%m-%y-%H:%M:%S"`
    tcpdump -n -s0 -c 5000 -w $dumpdir/TCPDUMP/pcap/$dateinfo.pcap
  ...
  tshark -r /root/TCPDUMP/pcap/$dateinfo.pcap -T fields -e ip.proto > /root/TCPDUMP/extra/protos.txt
  ...
  tshark -r $dumpdir/TCPDUMP/pcap/$dateinfo.pcap -T fields -E header=y -e ip.proto -e tcp.flags -e udp.srcport -e tcp.srcport -e data > /root/TCPDUMP/info/$dateinfo.txt
   ...
   sed -i 's/51.222.35.124,//g' /root/TCPDUMP/extra/sortedip.txt
   sed -i 's/51.222.35.124//g' /root/TCPDUMP/extra/sortedip.txt
